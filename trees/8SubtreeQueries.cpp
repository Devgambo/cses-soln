...
void seg_update(int node, int lo, int hi, int pos, ll val) {
    if (lo == hi) {
        seg[node] = val;
        return;
    }
...
void update(int s, ll x) {
    val[s] = x;
    seg_update(1, 1, n, tin[s], x);
}

ll sumSubtree(int s) {
    return seg_query(1, 1, n, tin[s], tout[s]);
}
...
int main() {
    ...
    // Build segment tree: place val[node] at position tin[node]
    vll flat(n+1, 0);
    for(int i = 1; i <= n; i++)
        flat[tin[i]] = val[i];
    
    //why does a segment tree have 4N size ? 
    seg.resize(4*(n+1), 0);
    build(1, 1, n, flat);
